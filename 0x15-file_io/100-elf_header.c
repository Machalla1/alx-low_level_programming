...
/**
 * check_elf - Look if a file is an ELF file.
...
void check_elf(unsigned char *e_ident)
{
	...
	for (indeex = 0; indeex < 4; indeex++)
	{
		if (e_ident[indeex] != 127 &&
...
void print_magic(unsigned char *e_ident)
{
	...
	for (indeex = 0; indeex < EI_NIDENT; indeex++)
	{
		printf("%02x", e_ident[indeex]);

		if (indeex == EI_NIDENT - 1)
			printf("\n");
		else
			printf(" ");
	}
...
/**
 * print_class - Output the class of an ELF header.
...
/**
 * print_data - Outout the data of an ELF header.
...
/**
 * print_abi - Output the ABI version of an ELF header.
...
/**
 * print_entry - Outpute the entry point of an ELF header.
